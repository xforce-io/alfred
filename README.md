# Alfred EverBot

[English](README_EN.md) | 中文

**Ever Running Bot** — 永远在线的个人 AI Agent 平台。

EverBot 让你的 AI Agent 像一个真正的助手一样持续运行：它能主动执行任务、通过多种渠道与你沟通、并随着交互不断积累记忆。你只需用 Markdown 定义 Agent 的行为和任务，剩下的交给 EverBot。

## 特性

- **永远在线**: 后台守护进程，Agent 7x24 待命，内置看门狗保活
- **心跳驱动**: Cron / Interval 定时自我唤醒，主动推进待办任务，支持时区感知与夜间降频
- **多渠道接入**: Web 界面（FastAPI + WebSocket）+ Telegram Bot，随时随地对话
- **技能系统**: 可扩展的插件化技能（代码审查、浏览器自动化、投资分析、论文检索等）
- **技能生命周期**: 技能自动评估→决策→更新，支持 Canary 发布与回滚
- **持久记忆**: 对话历史自动持久化，LLM 自动提取关键事实归档到 MEMORY.md
- **Markdown 驱动**: 用 AGENTS.md 定义人设，HEARTBEAT.md 定义任务，所见即所得

## 快速开始

```bash
git clone <repo-url> alfred && cd alfred
bin/setup                        # 安装环境（需要 Python 3.10+）
source .venv/bin/activate        # 激活虚拟环境
./bin/everbot init my_agent      # 创建 Agent（自动注册到配置）
./bin/everbot start              # 启动（daemon + Web）
```

> 如果系统默认 Python 不是 3.10+，可以指定版本：`PYTHON=python3.11 bin/setup`

> 详见 [QUICKSTART.md](QUICKSTART.md)，包含 Telegram 配置等完整步骤。

## CLI 命令

```bash
# 初始化
./bin/everbot init [agent_name]           # 创建 Agent 工作区

# 启动和管理
./bin/everbot start                       # 启动 daemon + web（后台）
./bin/everbot stop                        # 停止所有服务
./bin/everbot restart                     # 重启（macOS 走 launchctl，避免与看门狗竞争）
./bin/everbot status                      # 查看状态

# macOS 后台常驻（LaunchAgent）
./bin/everbot service-install             # 安装 LaunchAgent，登录自启
./bin/everbot service-uninstall           # 卸载 LaunchAgent
./bin/everbot service-status              # 查看 LaunchAgent 状态

# 其他
./bin/everbot list                        # 列出所有 Agent
./bin/everbot doctor                      # 环境自检（配置/技能/依赖/工作区）
./bin/everbot heartbeat --agent my_agent  # 手动触发心跳
./bin/everbot heartbeat --agent my_agent --force  # 忽略活跃时段限制
./bin/everbot config --show               # 显示当前配置
./bin/everbot config --init               # 初始化默认配置
./bin/everbot migrate-agent --agent my_agent  # 已废弃:dolphin 已移除,milkie 不使用 agent.dph
./bin/everbot cleanup-logs                # 清理历史日志并迁移旧 skill_logs
./bin/everbot memory stats --agent my_agent  # 查看 memory 系统统计

# 技能管理
./bin/everbot skills list                 # 列出所有技能
./bin/everbot skills search <query>       # 搜索技能
./bin/everbot skills install <source>     # 安装技能
./bin/everbot skills update [skill_name]  # 更新技能
./bin/everbot skills remove <skill_name>  # 删除技能
./bin/everbot skills enable <skill_name>  # 启用技能
./bin/everbot skills disable <skill_name> # 禁用技能
```

运行时文件：
- `~/.alfred/everbot.pid`: 守护进程 PID 文件
- `~/.alfred/everbot-web.pid`: Web 进程 PID 文件
- `~/.alfred/everbot.status.json`: 守护进程状态快照（供 `status`/Web 读取）
- `~/.alfred/logs/everbot.out`: 守护进程日志
- `~/.alfred/logs/everbot-web.out`: Web 服务器日志
- `~/.alfred/logs/heartbeat.log`: 心跳日志

## 技能系统

EverBot 通过可插拔的技能模块扩展 Agent 的能力。每个技能是一个独立目录，包含 `SKILL.md`（说明文档）和实现代码。

| 技能 | 用途 | 主要功能 |
|------|------|----------|
| **routine-manager** | 任务调度管理 | Cron/Interval 调度、时区感知、执行模式配置 |
| **invest** | 统一投资分析 | 多源信号采集（宏观流动性、价值评分、中国市场、箱体突破）、因果链条、资产概率分布 |
| **gray-rhino** | 风险趋势预警 | 新闻聚类、灰犀牛识别、资产影响映射 |
| **daily-attractor** | 每日市场监控 | 融资定价偏移、吸引子追踪、Telegram 推送 |
| **paper-discovery** | AI/ML 论文发现 | HuggingFace + arXiv 集成、热度评分、GitHub Star 排名 |
| **dev-browser** | 浏览器自动化 | 持久页面状态、ARIA 快照、截图能力 |
| **web** | 网络搜索与浏览器 | 多后端搜索（DuckDuckGo/Tavily）、页面提取、Playwright 自动化 |
| **skill-installer** | 动态技能管理 | 注册表安装、多源支持（Git/URL/本地） |
| **ops** | 运维与可观测性 | 状态监控、心跳管理、日志分析、生命周期管理、诊断 |
| **trajectory-reviewer** | 轨迹审查 | 日志分析、失败检测、循环识别、延迟定位 |

## 使用示例

### 示例 1: 基础示例

```bash
# 运行基础示例（展示所有功能）
PYTHONPATH=. python examples/everbot_demo.py
```

### 示例 2: 真实 Agent 对话

```bash
# 创建并与真实 Agent 对话
PYTHONPATH=. python examples/real_agent_demo.py

# 查看 Agent 信息
PYTHONPATH=. python examples/real_agent_demo.py info
```

### 示例 3: 在代码中使用

```python
from src.everbot import create_agent, UserDataManager
from pathlib import Path

# 初始化
user_data = UserDataManager()
user_data.init_agent_workspace("my_agent")

# 创建 Agent
agent_dir = user_data.get_agent_dir("my_agent")
agent = await create_agent("my_agent", agent_dir)

# 对话
async for event in agent.continue_chat(message="Hello!", stream_mode="delta"):
    # 处理响应...
    pass
```

## 工作区文件说明

### AGENTS.md - 行为规范

定义 Agent 的身份、职责和沟通风格：

```markdown
# Agent 行为规范

## 身份
你是 XXX 助理，负责...

## 核心职责
1. ...
2. ...

## 沟通风格
- 简洁专业
- 数据驱动
```

### HEARTBEAT.md - 心跳任务清单

定义定期执行的任务：

```markdown
# 心跳任务

## 待办
- [ ] 任务1
- [ ] 任务2

## 已完成
- [x] 任务0 (2026-02-01)
```

### Agent 行为来源

Agent 的行为不再由单一定义文件描述，而是由两部分共同决定：

- **工作区 Markdown**：`~/.alfred/agents/<name>/` 下的 `SOUL.md / AGENTS.md / SKILLS.md / USER.md / MEMORY.md`，由 `WorkspaceLoader` 读取并合成 system prompt。
- **milkie agent.md**：spawn `milkie serve` 时落在 `~/.alfred/milkie/<agent>/agent.md`，由 `systemPrompt`/FSM/model 字段决定 milkie 侧的运行行为。

> 历史遗留的 `agent.dph` 文件可能仍存在，但已不再被解析——agent runtime 已由 dolphin 替换为 milkie。

## 测试

```bash
# 运行所有测试
python -m pytest tests/ -v

# 按类型运行
python -m pytest tests/unit/ -v           # 单元测试（隔离，无外部依赖）
python -m pytest tests/integration/ -v    # 集成测试（跨模块，可能需要网络）
python -m pytest tests/e2e/ -v            # 端到端测试（WebSocket/API）

# 使用统一入口脚本
tests/run_tests.sh unit                   # 运行单元测试
tests/run_tests.sh all --coverage         # 运行全部测试并生成覆盖率

# 运行特定测试
python -m pytest tests/unit/test_agent_factory.py -v
```

测试覆盖（共 ~98 个测试文件）：

- **单元测试** (~64 个): Agent 工厂、Session 管理、内存系统、Channel 路由、心跳约束、Telegram 安全、Web 认证、技能加载、进程管理、工作流引擎等
- **集成测试** (~18 个): Daemon 生命周期、Session 恢复与锁、心跳执行流程、深度审查流程、工作区指令恢复、Token 优化、工作流端到端等
- **CLI 测试** (~3 个): 命令行入口、Session 管理、Chat 命令
- **端到端测试** (~7 个): WebSocket 对话（正常/中断/多会话）、Session 重置 API、故障兜底、运维端到端

## 架构

```
EverBot Daemon
    │
    ├── MilkieProvider ───────► milkie sidecar (`milkie serve` 子进程, HTTP + SSE)
    │                               │
    │                          discover_skills (扫工作区 SKILL.md)
    │
    ├── HeartbeatRunner (Agent A)
    │   ├── 读取 HEARTBEAT.md
    │   ├── RoutineManager (Cron/Interval 调度)
    │   ├── Inspector (执行监控、周期性推送)
    │   ├── 注入 Context
    │   ├── 执行 Agent
    │   └── 持久化 Session
    │
    ├── Skill Lifecycle Manager (SLM)
    │   ├── SegmentLogger (评估段日志)
    │   ├── LLM Judge (评分与评估)
    │   ├── DecisionEngine (更新/回滚决策)
    │   └── VersionManager (版本追踪)
    │
    ├── MemorySystem (双层)
    │   ├── ProfileExtractor + ProfileStore  (长期画像 → MEMORY.md)
    │   ├── EventExtractor  + EventStore     (时点事件 → events/YYYY-MM.md，append-only)
    │   ├── MemoryMerger (token 相似度去重、reinforce 上限 0.95、profile/event 双套衰减)
    │   └── memory-review job (每 2h 跑：合并/弃用/压缩到 USER.md，带 entropy 约束)
    │
    ├── ChannelService
    │   ├── SessionResolver (渠道→Agent 映射)
    │   ├── TelegramChannel
    │   └── WebChannel (FastAPI + WebSocket)
    │
    ├── SessionManager (JSONL 持久化 + 并发锁)
    │
    └── Web Dashboard (FastAPI)
        ├── Chat API (WebSocket 实时对话)
        ├── Agent/Session 管理 API
        └── API Key 认证
```

核心组件：

- **MilkieProvider**: 唯一的 agent runtime provider，跨进程驱动 `milkie serve` 子进程（HTTP + SSE），按工作区指令合成 system prompt
- **UserDataManager**: 统一数据管理（工作区、配置、日志）
- **WorkspaceLoader**: 工作区文件加载（AGENTS.md, HEARTBEAT.md, MEMORY.md, USER.md）
- **SessionManager**: Session 管理（JSONL 持久化、并发锁、Session 恢复）
- **MemoryManager**: 双层记忆编排器（profile 画像 + event 时点事件并行抽取、token 去重、reinforce/decay、auto-review 周期收敛、USER.md 画像压缩）
- **ChannelService**: 多渠道接入（Telegram Bot、Web UI）
- **RoutineManager**: 任务调度（Cron 表达式、Interval、时区感知、夜间降频）
- **HeartbeatRunner**: 心跳执行器（任务读取、上下文注入、结果持久化）
- **Inspector**: 执行监控（周期性强制推送、静默检测）
- **SLM**: 技能生命周期管理（评估→评分→决策→版本追踪）
- **EverBotDaemon**: 守护进程主逻辑（多 Agent 管理、信号处理、状态快照）

## 项目结构

```
alfred/
├── src/everbot/              # EverBot 核心模块
│   ├── cli/                  # CLI 入口
│   ├── channels/             # 渠道适配层
│   ├── web/                  # Web 服务（FastAPI + WebSocket）
│   │   ├── app.py            # FastAPI 应用
│   │   ├── auth.py           # API Key 认证
│   │   ├── services/         # Agent/Chat 服务层
│   │   ├── static/           # 静态资源
│   │   └── templates/        # 页面模板
│   ├── core/                 # 业务逻辑
│   │   ├── agent/            # Agent provider & milkie sidecar 集成
│   │   ├── channel/          # 多渠道接入（Telegram, Web）
│   │   ├── jobs/             # 后台作业
│   │   ├── memory/           # 记忆系统（提取、合并、存储）
│   │   ├── models/           # 系统事件模型
│   │   ├── runtime/          # 心跳执行、Turn 编排、调度器、Inspector
│   │   ├── scanners/         # 扫描器（任务发现等）
│   │   ├── session/          # Session 持久化、压缩、历史管理
│   │   ├── slm/              # 技能生命周期管理（SLM）
│   │   ├── tasks/            # 任务调度（RoutineManager）
│   │   └── workflow/         # 工作流引擎
│   └── infra/                # 基础设施（配置、工作区、进程管理）
│
├── skills/                   # 可扩展技能模块
│   ├── routine-manager/      # 任务调度管理
│   ├── invest/               # 统一投资分析（含宏观流动性、价值评分、中国市场信号）
│   ├── gray-rhino/           # 风险趋势预警
│   ├── daily-attractor/      # 每日市场监控
│   ├── paper-discovery/      # AI/ML 论文发现
│   ├── dev-browser/          # 浏览器自动化
│   ├── web/                  # 网络搜索与浏览器自动化
│   ├── skill-installer/      # 动态技能安装
│   ├── ops/                  # 运维与可观测性
│   └── trajectory-reviewer/  # 轨迹审查
│
├── tests/                    # 测试（~98 个测试文件）
│   ├── unit/                 # 单元测试（隔离，无外部依赖）
│   ├── integration/          # 集成测试（跨模块，可能需要网络）
│   ├── cli/                  # CLI 测试
│   └── e2e/                  # 端到端测试（WebSocket/API）
│
├── docs/                     # 设计与技术文档
│   ├── EVERBOT_DESIGN.md     # 架构设计
│   ├── runtime_design.md     # 运行时设计
│   ├── memory_system_design.md # 记忆系统设计
│   ├── channel_design.md     # 多渠道设计
│   ├── skill_lifecycle_design.md # 技能生命周期设计
│   ├── SKILLS_GUIDE.md       # 技能开发指南
│   ├── glossary.md           # 术语表
│   └── skills/               # 技能级设计文档
│
├── examples/                 # 使用示例
│   ├── everbot_demo.py       # 基础功能演示
│   └── real_agent_demo.py    # 真实 Agent 对话示例
│
├── config/                   # 配置模板
│   └── everbot.example.yaml  # EverBot 配置示例
│
├── bin/                      # 可执行脚本
│   ├── everbot               # CLI 入口
│   ├── everbot-watchdog      # 看门狗脚本
│   └── setup                 # 安装脚本
│
└── requirements.txt          # Python 依赖
```

## 常见问题

### Q: 如何修改心跳间隔？

A: 编辑 `~/.alfred/config.yaml`，修改 `everbot.agents.<agent_name>.heartbeat.interval` 值（单位：分钟）。还可以设置 `heartbeat.night_interval` 来降低夜间频率（0 或省略表示夜间静默）。`active_hours` 按部署机本地时区判断；改完需 `./bin/everbot restart` 生效。

### Q: 心跳任务会污染用户对话历史吗？

A: 不会。默认使用 `isolated` 模式，心跳使用独立的 Session（`heartbeat_<agent_name>`）。

### Q: History 会无限增长吗？

A: 不会。`HistoryManager` 会自动裁剪过长的历史，保留最近 10 轮对话，其余归档到 `MEMORY.md`。MemorySystem 会自动提取关键事实并去重。

### Q: 如何查看心跳日志？

A: 查看 `~/.alfred/logs/heartbeat.log`。

### Q: 如何自定义 Agent 的行为？

A: 编辑 `~/.alfred/agents/<agent_name>/` 下的工作区 Markdown（`SOUL.md / AGENTS.md / SKILLS.md / USER.md / MEMORY.md`）来塑造 Agent 行为；它们会被合成为 system prompt 并在 spawn milkie sidecar 时传入。

### Q: 如何开发自定义技能？

A: 参考 [docs/SKILLS_GUIDE.md](docs/SKILLS_GUIDE.md)，在 `skills/` 下创建新目录，包含 `SKILL.md` 和实现代码。

## Roadmap

- [ ] macOS launchd 深度集成
- [ ] Metrics 和监控告警
- [ ] 多用户权限管理
- [ ] 技能市场（远程注册表）
- [ ] 高级记忆功能（RAG、向量检索）

## 许可证

（待定）

## 参考

- 架构设计: [docs/EVERBOT_DESIGN.md](docs/EVERBOT_DESIGN.md)
- 运行时设计: [docs/runtime_design.md](docs/runtime_design.md)
- 记忆系统: [docs/memory_system_design.md](docs/memory_system_design.md)
- 多渠道设计: [docs/channel_design.md](docs/channel_design.md)
- 技能生命周期: [docs/skill_lifecycle_design.md](docs/skill_lifecycle_design.md)
- 技能开发: [docs/SKILLS_GUIDE.md](docs/SKILLS_GUIDE.md)
