...
| **去 dolphin 硬依赖** | 拆 `import dolphin` 与依赖声明;milkie 成为唯一 runtime | ✅ **彻底完成**(经真实 LLM 验证 milkie 可接管生产后执行):删 `provider/dolphin/` 整包 + factory shim;provider milkie-only + 翻默认;oneshot `call_llm` → dolphin-free `OneshotLLMProvider`;模型配置 → `model_config`(脱 dolphin factory);配置工具 → `agent_config`;`dolphin_compat` 纯化;telegram_skillkit 降级纯发送类。**全仓零 `import dolphin`**(`test_no_dolphin_imports_anywhere` 反转守护)。1606 unit 绿 + 6 milkie e2e 绿 + dolphin-blocked 导入证伪 + 真 LLM e2e。删 dolphin 相关测试 + 净删 ~3900 行 |
