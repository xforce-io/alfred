...
| **去 dolphin 硬依赖** | 拆 `import dolphin` 与依赖声明;保留 DolphinProvider 作 fallback | ⬜ 最后(依赖 E + telegram 原生化;**不再依赖 milkie#87**) |
...
- **去依赖(可证伪验收)**:边界守护测试反转 —— `grep -rn "import dolphin\|from dolphin" src/everbot`(排除 DolphinProvider fallback 的隔离 import)**零命中** + requirements 移除 dolphin;全套 unit + 真 e2e 回归绿。
