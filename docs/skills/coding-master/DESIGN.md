...
cm claim           cm dev              cm test            cm done
   │                 │                    │                  │
   ▼                 ▼                    ▼                  ▼
┌─────────┐    ┌───────────┐    ┌──────────────┐      ┌──────────┐
│ pending │───►│ analyzing │───►│  developing  │─────►│   done   │
...
| 6 | `cm done --feature N` | — | → `done` | `completed_at` | evidence 通过，test_commit=HEAD |
| 7 | `cm integrate` | → `integrating` | — | `integration_passed_at` | 所有 feature done |
| 8 | `cm submit` | → `done` | — | PR URL | 集成测试通过 |
...
- 不支持创建新文件——只编辑已有文件，减少 agent 随意创建文件的风险
