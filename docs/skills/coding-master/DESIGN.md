...
| `cm report [--content C] [--file F]` | 写入报告或诊断（review/debug/analyze 模式） |
| `cm engine-run [--goal G] [--engine E] [--timeout T]` | 委托引擎子进程执行代码分析 |
...
Engine 层让 CM agent 从"逐文件手动分析"退化回归到"调度器"角色。agent 定义 scope，委托 engine 子进程完成分析，只需消化结果、写 report。
