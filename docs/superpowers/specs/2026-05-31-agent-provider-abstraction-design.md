...
1. **`provider/dolphin/factory.py`**：现 `core/agent/factory.py` 用 `git mv` 整体迁入（保留 git 历史），**内部逻辑一字不改**。`core/agent/factory.py` 改为薄 re-export shim，保留 `AgentFactory` / `create_agent` / `get_agent_factory` 旧导入路径及其静态方法（`_resolve_agent_model` / `_append_runtime_paths` / `_get_global_config` 等被 cron/heartbeat/core_service/cli 直接引用，必须经类完整 re-export 保持可用）。`core/agent/factory.py` 内部对 `infra/dolphin_compat` 的 import 随迁移改为指向 `provider/dolphin/compat`（值不变）。
2. **`provider/dolphin/compat.py`**：现 `infra/dolphin_compat.py` 的 dolphin 部分（`flags` + 常量 + `ensure_continue_chat_compatibility`）迁入；`infra/dolphin_compat.py` 改为薄 re-export shim。这样被它依赖的 8 处主干文件（heartbeat/inspector/factory/core_service/skill_change_detector/context_manager/session/agent_service）与 `test_dolphin_compat.py`、`test_skill_change_detector.py` 全部无需改动。注意 `test_dolphin_compat.py` 用 `@patch("src.everbot.infra.dolphin_compat.flags")`，故 shim 必须仍持有可被 patch 的 `flags` 引用（即 `ensure_continue_chat_compatibility` 的真实实现仍在 `infra/dolphin_compat.py`，或 shim 重新导出 `flags`）。
