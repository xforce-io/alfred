...
```
cm lock        PLAN.md 创建      cm claim       全部 done      cm submit
   │                │                │               │              │
   ▼                ▼                ▼               ▼              ▼
┌────────┐    ┌──────────┐    ┌──────────┐    ┌───────────┐    ┌──────┐
│ locked │───►│ planning │───►│ working  │───►│ completed │───►│ done │
└────────┘    └──────────┘    └──────────┘    └───────────┘    └──────┘
```

| session_phase | 含义 | 进入条件 | cm progress 显示 |
|---------------|------|----------|------------------|
| `locked` | workspace 锁定，尚未规划 | `cm lock` | "创建 PLAN.md 定义 feature 列表" |
| `planning` | PLAN.md 已创建 | 工具检测到 PLAN.md 存在 | "认领 feature 开始开发" |
| `working` | 有 feature 在进行中 | 第一个 `cm claim` | 显示各 feature 状态 |
| `completed` | 所有 feature 都 done | 工具检测 claims.json | "运行 cm submit 提交" |
...
`session_phase` 由工具在关键操作时自动更新（`cm lock` → locked，`cm claim` → working 等），agent 不需要手动管理。`cm progress` 同时展示 session 级状态和 feature 级状态。
...
- `cm done` 的前置条件：`test_status == passed && test_commit == latest_commit`（测试通过且之后没有新 commit）
...
`cm progress` 从每个阶段的子状态计算展示信息和指引：
...
Feature 2 [developing] dolphin-a
  ...
  → 阅读 features/02-incremental-scan.md 了解上下文，修复测试失败，commit 后运行 cm test --feature 2
...
Feature 3 [developing] dolphin-b
  ...
  → 阅读 features/03-report-generator.md 确认 Acceptance Criteria 全部满足，然后运行 cm done --feature 3
...
1. **去哪工作** — worktree 路径
2. **读什么** — feature_md 路径（包含 Analysis/Plan/AC/Dev Log）
3. **现在什么状况** — 各阶段子状态 + 测试输出摘要
4. **下一步做什么** — 自然语言指引（含具体命令）

**指引生成规则**：

| phase | 子状态 | 指引 |
|-------|--------|------|
| `analyzing` | `analysis == pending` | 阅读 `{feature_md}` 中的 Spec，在 Analysis 段落分析代码 |
| `analyzing` | `analysis == done, plan == pending` | 在 `{feature_md}` 中撰写 Plan |
| `analyzing` | `analysis == done, plan == done` | 分析完成，运行 `cm dev --feature N` |
| `developing` | `test_status == pending` | 在 `{worktree}` 中开发，commit 后运行 `cm test --feature N` |
| `developing` | `test_status == failed` | 阅读 `{feature_md}` 了解上下文，修复测试失败（{test_output}），commit 后运行 `cm test --feature N` |
| `developing` | `test_status == passed, test_commit ≠ latest` | 代码已变更，运行 `cm test --feature N` 重新测试 |
| `developing` | `test_status == passed, test_commit == latest` | 阅读 `{feature_md}` 确认 Acceptance Criteria 全部满足，然后运行 `cm done --feature N` |
| `done` | — | 已完成 |

#### 3.4.5 为什么是 JSON 不是 MD
...
- **cm 工具自动追加关键事件** — lock/claim/done/submit 时工具层自动往 JOURNAL.md 追加一行，agent 可以在自由时机补充上下文
...
- 每个 feature 有独立的 branch、worktree 和 feature lock
- agent 只在自己 feature 的 worktree 内修改代码和跑测试
- `claims.json` 解决“谁负责哪个 feature”，worktree/lock 解决“谁在改哪份代码”
...
2. **规划** — 创建 PLAN.md，定义 feature 列表和验收标准（session: planning）

**Feature 级**（每个 feature 重复）：
3. **认领** — `cm claim --feature <n>`（feature: pending → analyzing，session: working）
4. **分析** — 在 `features/XX.md` 中撰写 Analysis + Plan
5. **进入开发** — `cm dev --feature <n>`（feature: analyzing → developing）
6. **开发** — 在 worktree 里改代码，commit
7. **测试** — `cm test --feature <n>`（通过：test_status=passed / 失败：test_status=failed）
8. **修复循环** — 测试失败 → 读 test_output → 改代码 → commit → `cm test` → 直到通过
9. **完成** — `cm done --feature <n>`（feature: developing → done，需 test_status=passed 且 test_commit=latest_commit）
10. **继续** — 认领下一个可用 feature，重复 3-9

**收尾**：
11. **查看进度** — `cm progress` 展示 session + 各 feature 的状态和自然语言指引
12. **提交** — 全部 feature done 后 `cm submit`（session: completed → done）
...
| `cm done --feature <n>` | 检查 test_status=passed 且 test_commit=latest_commit → developing → done |
| `cm progress` | 展示 session 状态 + 各 feature 阶段/子状态 + 自然语言指引 |
| `cm submit --repo <name> --title "..."` | 幂等提交：汇总 feature 分支 → commit → push → PR，成功后自动 unlock |
...
7. **先 cm test 再 cm done** — cm done 不跑测试，只检查 developing 子状态中 test_status=passed 且 test_commit=latest_commit；代码改了必须重新 cm test
8. **开发完释放锁**
...
def _atomic_json_update(path: Path, updater):
    ...
    with open(path, "r+") as f:
        ...
        try:
            ...
            snapshot = json.dumps(data)  # 快照，用于回滚判断
            result = updater(data)
...
def cmd_lock(args) -> dict:
    ...
    def reserve_lock(data):
        ...
        branch = args.branch or f"dev/{args.repo}-{datetime.now().strftime('%m%d-%H%M')}"
        reserved.update({
            "repo": args.repo,
...
def cmd_claim(args) -> dict:
    ...
    for dep in deps:
        ...
        if dep_phase != "done":
            return {"ok": False, "error": f"blocked: Feature {dep} is {dep_status}"}
    ...
    # 更新 session_phase：首次 claim 时 → working；同时将 agent 加入 session_agents
    lock_path = repo / CM_DIR / "lock.json"
    def update_session(data):
        if data.get("session_phase") in ("locked", "planning"):
...
def cmd_done(args) -> dict:
    ...
    3. developing.test_commit == developing.latest_commit（代码没有在测试后变更）
    ...
    def do_done(data):
        ...
        if dev.get("test_commit") != dev.get("latest_commit"):
            return {"ok": False, "error": f"code changed after last test "
                    f"(tested {dev.get('test_commit', '?')[:7]}, latest {dev.get('latest_commit', '?')[:7]}), run cm test again"}
        ...
        # 检查是否所有 feature 都 done → session_phase = completed
    ...
    if result.get("ok") and result.get("data", {}).get("all_done"):
        lock_path = repo / CM_DIR / "lock.json"
        def mark_completed(data):
            data["session_phase"] = "completed"
            return {"ok": True}
        _atomic_json_update(lock_path, mark_completed)
    return result

# ── cm progress ───────────────────────────────────────

def cmd_progress(args) -> dict:
    """读 lock.json + claims.json + PLAN.md，返回两级状态 + 自然语言指引。
    ...
    # 自动推进 session_phase：locked → planning（检测到 PLAN.md 存在）
    if lock.get("session_phase") == "locked" and plan_path.exists() and plan:
        def to_planning(data):
            if data.get("session_phase") == "locked":
                data["session_phase"] = "planning"
            return {"ok": True}
        _atomic_json_update(lock_path, to_planning)
        lock["session_phase"] = "planning"
    ...
    for fid, spec in plan.items():
        ...
        next_action = _generate_next_action(phase, claim, fid, feature_md, blocked_by if phase == "blocked" else [])
...
def _generate_next_action(phase, claim, fid, feature_md, blocked_by) -> str:
    """根据 phase + 子状态生成精确的自然语言指引。"""
    if phase == "blocked":
        return f"等待依赖完成: {', '.join(f'Feature {d}' for d in blocked_by)}"
    if phase == "pending":
        return f"可认领，运行 cm claim --feature {fid}"
    if phase == "analyzing":
        a = claim.get("analyzing", {})
        if a.get("analysis") != "done":
            return f"阅读 {feature_md} 中的 Spec，在 Analysis 段落分析代码"
        if a.get("plan") != "done":
            return f"在 {feature_md} 中撰写 Plan"
        return f"分析完成，运行 cm dev --feature {fid}"
    if phase == "developing":
        dev = claim.get("developing", {})
        ts = dev.get("test_status", "pending")
        if ts == "pending":
            return f"在 {claim.get('worktree')} 中开发，commit 后运行 cm test --feature {fid}"
        if ts == "failed":
            output = dev.get("test_output", "")[:100]
            return f"阅读 {feature_md} 了解上下文，修复测试失败（{output}），commit 后运行 cm test --feature {fid}"
        if ts == "passed":
            if dev.get("test_commit") != dev.get("latest_commit"):
                return f"代码已变更，运行 cm test --feature {fid} 重新测试"
            return f"阅读 {feature_md} 确认 Acceptance Criteria 全部满足，然后运行 cm done --feature {fid}"
    return "已完成"
...
def _generate_suggestions(features: list, lock: dict) -> list:
    ...
    if all_done:
        suggestions.append("所有 feature 已完成，运行 cm submit 提交")
        return suggestions
    for f in features:
        if f["phase"] == "pending":
...
def cmd_submit(args) -> dict:
    """幂等提交：每一步都检查是否已完成，崩溃后重跑安全。

    执行顺序：
    1. 检查是否所有 feature 都 done
    2. 切到 dev branch，按依赖拓扑序 merge feature branches
    3. git add + commit（跳过如果 working tree clean）
    4. git push（跳过如果远端已有且一致）
    5. gh pr create（跳过如果 PR 已存在）
    6. 清理 feature worktrees（失败仅警告，cm doctor 兜底）
    7. unlock（失败不阻塞返回，仅警告）
    """
    repo = _resolve_locked_repo(args)
    plan = _parse_plan_md(repo / CM_DIR / "PLAN.md")
    claims = _atomic_json_read(repo / CM_DIR / "claims.json")
    ...
    # Step 2: 切到 dev branch，按依赖拓扑序 merge feature branches
    _run_git(repo, ["checkout", branch])
    feature_branches = [
        claims["features"][fid]["branch"]
        for fid in _topo_sort(plan)  # 按依赖拓扑排序，保证先 merge 基础 feature
        if "branch" in claims["features"].get(fid, {})
    ]
    for fb in feature_branches:
        merge_result = _run_git(repo, ["merge", fb, "--no-edit"], check=False)
        if "CONFLICT" in (merge_result or ""):
            # merge 冲突：中止 merge，返回错误让 agent 手动解决
            _run_git(repo, ["merge", "--abort"], check=False)
            return {"ok": False, "error": f"merge conflict when merging {fb} into {branch}. "
                    f"Resolve in the feature worktree (rebase onto {branch} or merge {branch} into feature branch), "
                    f"commit, then re-run cm submit."}

    # Step 3: commit（幂等 — working tree clean 则跳过）
    ...
    # Step 6: 清理 feature worktrees（失败仅警告）
    for fid in plan:
    ...
    # Step 7: session_phase → done + unlock（失败仅警告，不阻塞成功返回）
    try:
        def mark_done(data):
...
def _generate_pr_body(repo: Path) -> str:
    ...
    if journal_path.exists():
        ...
        for line in journal_path.read_text().splitlines():
            # 只提取里程碑条目（done/submit）
            if any(kw in line for kw in ["done", "submit", "plan"]):
                lines.append(line)
        lines.append("")
...
1. 读 `PLAN.md` → `_parse_plan_md` 解析出 4 个 feature
2. **预检查**（只读）：读 `claims.json` → Feature 1 未被认领 ✓，无依赖 ✓
...
**返回给 Agent B**：`{"ok":false, "error":"already analyzing by dolphin-a"}`
...
**工具层**：`cmd_done` 执行：
1. 获取 Feature 1 worktree 的当前 HEAD → `a1b2c3d`
2. 对 `claims.json` 执行 `_atomic_json_update`（flock 加锁）：
   - 计算有效状态 = `testing`（test_commit = HEAD）✓
   - Feature 1 status `testing` → `done`
   - 写入 `completed_at`
...
// claims.json
{
  "features": {
    "1": {
      ...
      "phase": "done",
      "claimed_at": "2026-03-08T10:00:00Z",
      "dev_started_at": "2026-03-08T10:15:00Z",
      "test_commit": "a1b2c3d",
      "test_passed_at": "2026-03-08T10:41:00Z",
      "completed_at": "2026-03-08T10:42:00Z"
    }
...
**工具层**：读 lock.json + claims.json + PLAN.md，展示两级状态 + 自然语言指引：
...
```json
{
  ...
  "data": {
    ...
    "total": 4, "done": 4,
    "features": [
      {"id": "1", "title": "提取 SessionScanner 接口", "phase": "done", "agent": "dolphin-a", "next": "已完成"},
      {"id": "2", "title": "实现增量 scan", "phase": "done", "agent": "dolphin-a", "next": "已完成"},
      {"id": "3", "title": "拆分 ReportGenerator", "phase": "done", "agent": "dolphin-b", "next": "已完成"},
      {"id": "4", "title": "集成测试", "phase": "done", "agent": "dolphin-a", "next": "已完成"}
    ],
    "suggestions": ["所有 feature 已完成，运行 cm submit 提交"]
  }
}
```
...
1. 检查所有 feature done ✓
2. 切到 dev branch，按依赖拓扑序 merge feature branches（冲突则中止并报错）
3. `git add -A :(exclude).coding-master` → `git commit`（working tree clean 则跳过）
4. `git push -u origin dev/alfred-0308-1000`（远端一致则 no-op）
5. `gh pr create --title "..." --body "..."`（PR 已存在则跳过）
6. 自动执行 `cmd_unlock`（失败仅警告，不阻塞成功返回）
...
| JOURNAL.md 有完整时间线？ | ✓ lock/plan/claim/done/submit 关键事件均记录 |
| 工具调用次数合理？ | ✓ 全程：1 lock + 4 claim + N test + 4 done + 1 progress + 1 submit = ~11+N 次工具调用（N 为开发-测试循环次数），其余全是 agent 直接操作 |
| claim 写入顺序正确？ | ✓ 先创建 worktree/MD，最后原子写入 claims.json |
| worktree 基点正确？ | ✓ Feature 2/3 从 Feature 1 的 branch 创建，继承代码改动 |
| submit 幂等？ | ✓ 每一步都检查是否已完成，崩溃后重跑安全 |
| submit merge 按拓扑序？ | ✓ `_topo_sort` 保证先 merge 基础 feature，冲突时自动 abort |
...
| `cm submit`：merge feature branch 冲突 | merge 被中止，dev branch 状态不变 | 工具自动 `merge --abort`，返回冲突详情；agent 进入对应 feature worktree 解决冲突（rebase 到 dev branch 或手动 merge），commit 后重新 `cm submit` |
...
- `cm done`：重跑只检查 developing 子状态（test_status == passed && test_commit == latest_commit），不跑测试；满足则标记 done
...
| updater 返回 ok:false | updater 修改了 data 但返回失败 | 文件内容恢复快照，不写入修改 |
| updater 返回 ok:false 且未修改 data | updater 只读 data 后返回失败 | 文件内容不变，无多余写操作 |
...
def test_e2e_single_agent():
    """单 agent 完成 lock → plan → claim → dev → done → submit 全流程。"""
    ...
    assert Path(r["data"]["feature_md"]).exists()

    # 4. 开发（在 worktree 里修改代码）
    write_file(Path(r["data"]["worktree"]) / "foo.py", "def foo(): return 42")
...
| 测试类别 | 用例数 | 覆盖目标 |
|----------|--------|----------|
| 单元测试 | ~41 | 每个内部函数的正常/异常路径（含 cmd_test 状态写入、cmd_done 状态检查、_topo_sort、_resolve_agent、updater 回滚） |
| 并发测试 | ~5 | flock 原子性、竞态条件 |
| 崩溃恢复测试 | ~9 | §8.1 中每个崩溃点（含 cm test 崩溃） |
| 幂等测试 | ~8 | 每个工具的重复执行（含 test 幂等、done without test） |
...
### Phase 1：新建 + 并行
- 实现 tools.py（lock, unlock, claim, dev, test, done, progress, submit, journal, renew, doctor, status）
- 重写 SKILL.md 为公约模式
...
6. **两级状态机驱动流程** — session 级（locked → planning → working → completed → done）+ feature 级（pending → analyzing → developing → done），每个阶段有独立子状态和明确的退出条件；`cm progress` 将两级状态翻译为自然语言指引，支持接力 agent 无缝接手
7. **测试绑定 commit** — `cm test` 将测试结果（含输出摘要）写入 developing 子状态并绑定 commit SHA；代码变更后 test_commit ≠ latest_commit，`cm done` 拒绝，必须重新测试
8. **每个文件有且只有一个 owner** — 消除多方写同一文件的冲突（JOURNAL.md 例外：flock + append-only，无冲突）
9. **先副作用后提交** — 先创建可逆的副作用（worktree/文件），最后原子写入 JSON；崩溃时状态未变，残留由 doctor 清理
10. **工具幂等** — 所有工具崩溃后重跑安全，不会产生重复状态或副作用
11. **Lease 防竞态** — lock 有过期时间，长任务需 renew；过期后操作被拒绝，避免多方同时写
12. **自愈能力** — `cm doctor` 检测并修复所有已知的不一致状态，是崩溃恢复的最后防线
13. **单机约束** — 并发安全依赖 flock，仅保证同一台机器上的多 agent 原子性
14. **Dev branch 纯基线** — dev branch 在 session 期间不接受直接 commit，只做初始基线和最终 merge 汇总点
15. **失败不写入** — `_atomic_json_update` 在 updater 返回失败时恢复快照，防止意外的 data 修改泄露到文件
