...
3. 历史一致性由 milkie serve 自持久化保证（事件溯源 + checkpoint），跨进程导出经 `POST /session/history` 得到 canonical `Message[]`，工具调用链（`tool_calls → tool_response`）由 milkie 内部维护配对。
...
**TurnExecutor 的保留角色**：`TurnExecutor`（`core/runtime/turn_executor.py`）保留为 heartbeat/job session 的轻量 context strategy 分发层，负责 session 锁 + ContextStrategy 查表 + 经 `MilkieProvider.run_turn`（`POST /chat` SSE）驱动一轮。它不承担重试/budget 逻辑。

**关键点**：一轮对话由 `MilkieProvider.run_turn` 经 `POST /chat`（SSE）驱动：`SSEParser` 增量解析 → `milkie_event_to_progress` 适配成中立 `{"_progress":[...]}`。milkie 内部已处理 tool 调用（`tool.requested` / `tool.responded`）的多轮循环、streaming、上下文管理。**TurnOrchestrator 在中立 `_progress` 流上**增加重试、tool budget、failure signature 等运行时保护，以 `TurnEvent` 流的形式暴露给消费层。
...
| Sidecar | 复用 heartbeat agent 的 sidecar | 临时 contextId（隔离），复用同 agent sidecar |
| 可跳过 | 是（前台忙就跳过） | 否（排队等） |
...
└─────────┬────────────────┘
          ...
          ▼
┌──────────────────────────┐
│  3. Milkie Turn          │  milkie serve 内部管理
│                          │
│  POST /chat (SSE)        │
│    {contextId, input,    │
│     goal}                │
...
│                          │
│  - save session metadata │    (历史由 milkie serve 自持久化)
...
milkie agent 的 system prompt 由其 `agent.md` 的 `systemPrompt` 在 spawn 时确定，对话历史由 `milkie serve` 自持久化（按 contextId）。每轮的易变内容经 `POST /chat` 的 `input` 注入。因此 Strategy 的职责是**构建 system prompt（spawn 时）+ 每轮的 input**，而非完整 context：
...
class ContextStrategy(Protocol):
    def build_system_prompt(self, session: Session, deps: RuntimeDeps) -> str:
        """构建 system prompt，spawn 时落入 sidecar agent.md 的 systemPrompt"""
        ...
...
**与 milkie sidecar 的协作关系**：

```
TurnExecutor                          milkie serve (sidecar)
    │                                      │
    ├─ strategy.build_system_prompt()      │ (spawn 时落入 agent.md systemPrompt)
    ...
    │                                      │
    ├─ provider.run_turn(                  │
    │    POST /chat (SSE)              ────►│ milkie 内部：
    │    {contextId, input, goal}          │   1. 按 contextId 加载历史
    │  )                                   │   2. 追加 input
    │                                      │   3. 调用 LLM
    │  ◄── SSE: message_delta /            │   4. 处理 tool.requested → tool.responded
    │      tool.requested / tool.responded─│   5. 循环直到 agent.run.completed
    │      （SSEParser → _progress）       │   6. 自持久化历史（sqlite/jsonl）
    │                                      │
    ├─ 历史无需 alfred 回写                │
    │  (milkie serve 自持久化;            │
    │   需读取时 POST /session/history)    │
```
...
- **为什么不用 Session 继承？** Session 是纯数据对象（持久化状态：contextId, revision, mailbox；对话历史在 milkie serve 侧）。system_prompt 构建依赖 workspace、task_manager 等外部组件，如果塞进 Session 子类会让 Session 从数据对象膨胀为 God object，耦合运行时依赖。
...
- **History 由 milkie serve 管理**：不同 session type 的 history 策略差异（primary 完整保留、heartbeat 极短、job 独立积累）通过 contextId 隔离实现——primary 用稳定 contextId（跨重启自动从 checkpoint 恢复），heartbeat/job 用各自独立或临时的 contextId。这不是 Strategy 的职责，而是会话身份（contextId 对齐）逻辑的职责。
...
```
 ...
 ── 执行（Scheduler tick，到期）─────────────────────────────────────────────
         ...
         │                               │                               │
         │                        provider.run_turn() (POST /chat SSE)    │
         │                        (在 heartbeat contextId 内)             │
         │                               │                               │
         │                        结果: "该喝水了！"                      │
...
```
 ...
 ── 执行（次日 9:00，Scheduler tick）──────────────────────────────────────
         ...
         │                    │                                           │
         │             POST /chat (SSE): run_turn()                       │
         │               → 调用 run_command 抓取 HN                       │
         │               → tool.responded 返回                           │
         │               → 筛选文章、生成摘要                              │
         │               → agent.run.completed                           │
         │                    │                                           │
...
│  │  ├─ 读磁盘 session 元数据      │    │  │  ├─ 读磁盘 session 元数据   │
│  │  ├─ acquire sidecar           │    │  │  ├─ acquire sidecar         │
│  │  ├─ 读 HEARTBEAT.md            │    │  │  └─ (SidecarPool 复用)      │
│  │  ├─ run_turn (POST /chat)     │    │  │                             │
│  │  ├─ 写磁盘 session 元数据      │    │  ├─ 每条用户消息               │
│  │  └─ emit() → 空(_subscribers=0)│    │  │  ├─ 复用 sidecar             │
│  └─ 结果写 heartbeat.log          │    │  │  ├─ run_turn (POST /chat)    │
│                                   │    │  │  └─ 写磁盘 session 元数据    │
└───────────┬───────────────────────┘    └──┬─────────────────────────────┘
            │                               │
            ▼       共享文件系统 + milkie     ▼
   session 元数据 ◄── 协同通道 ──► session 元数据
   HEARTBEAT.md     (历史由 milkie serve     (同一文件)
                     按 contextId 自持久化)
```
...
3. **last-write-wins 覆盖**：daemon 写完 session 元数据后，Web 下次用过时的内存快照存盘，会覆盖 daemon 的心跳结果。
4. **锁不跨进程**：`asyncio.Lock` 只在 daemon 进程内有效。

### 5.2 To-Be（Phase A）：双进程一致性协议（先止血）

在现有部署现实下（daemon + web 并存），先建立**跨进程可验证的一致性协议**。核心原则：
`session.json`（会话元数据）是 alfred 侧的唯一真相源，内存快照只是缓存，不允许直接覆盖写盘。（对话历史另由 milkie serve 按 contextId 自持久化。）
...
| Web/daemon 各自 stale 快照 | 每轮写入前锁内重读最新 session，缓存只作为加速层 |
...
2. 心跳与聊天可复用同一 sidecar（SidecarPool 常驻），减少反复 spawn 成本。
3. 排障面更小（少一个进程边界）。
...
class Inspector:
    def __init__(self, *, ..., llm: Optional["OneshotLLMProvider"] = None, ...):
        self.llm = llm  # 无状态一次性 LLM provider

    async def _run_llm(self, prompt: str) -> str:
        """执行反思 LLM 调用（无状态 one-shot，不进会话历史）。"""
        # OneshotLLMProvider → POST /llm（payload 含 tier=fast + temperature）
...
### 6.4 工作区配置热更新（sidecar freshness）

**问题**：workspace 文件（SOUL/AGENTS/SKILLS/USER/MEMORY.md）改动后，常驻 sidecar 仍持有旧的
system prompt（spawn 时已落入 agent.md `systemPrompt`），新值不生效。

**根因**：milkie agent 的 system prompt 在 spawn 时确定，不随每轮 input 改变；常驻复用的 sidecar
不会自动感知磁盘配置变化。

**设计原则**：配置派生内容（system prompt、技能段）随 sidecar 生命周期管理，对话历史与运行态变量
分离：

| 类别 | 内容 | 更新策略 |
|------|------|---------|
| 配置派生（从磁盘文件构建） | system prompt、技能段 | sidecar **重生**时从磁盘重新构建落入 agent.md |
| 运行态（contextId 特有） | 对话历史、会话变量 | 由 milkie serve 按 contextId 自持久化，跨重生从 checkpoint 恢复 |

**实现**：`SidecarPool` 按**技能指纹**（过滤后 name/title/description/abs_path 的 sha256）做 freshness
检查，配置/技能变化时只重生该 agent 的 sidecar（重建 agent.md），免全量重启 daemon。重生后同
contextId 的历史自动从 checkpoint 恢复，新 system prompt 立即生效。
...
1. 任务可分为两类：
   ...
   - `llm`：需要推理和外部工具链
2. 对 `deterministic` 任务，HeartbeatRunner 直接生成结果并写入事件，不经过 `run_turn()`（不调 LLM）。
...
当前实现通过 CLI 脚本 `routine_cli.py` 暴露 HEARTBEAT.md 的 task CRUD。该脚本以 `SKILL.md` 形式被 `discover_skills` 发现，agent 在对话或心跳中经 milkie 内建 `run_command` 调用该脚本。
...
4. 经 milkie 内建 `run_command` 调用，与 SKILL.md 中声明的脚本统一发现与执行。
...
9. 实现 `ContextStrategy` Protocol + `PrimaryContextStrategy` + `HeartbeatContextStrategy`（基于 milkie agent.md systemPrompt + `POST /chat` 的 input 注入）。
10. `PrimaryContextStrategy.build_message()` 在有 mailbox 事件时 drain 并注入 message 前缀，保持 system_prompt 稳定（prefix cache 友好）。
11. 配置/技能变化触发 `SidecarPool` 重生该 agent sidecar，重建 agent.md system prompt（配置热更新）。
...
**配置热更新：**
14. 配置热更新：system prompt/技能段始终从磁盘重建并随 sidecar 重生生效，不被旧 session 状态覆盖。
...
3. **ContextStrategy 按 session type 分化推理流**：system prompt 保持稳定（prefix cache 友好，spawn 时落入 agent.md），per-turn 易变内容（mailbox events、due tasks）经 `POST /chat` 的 input 注入。
