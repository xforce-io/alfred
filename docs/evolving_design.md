...
2. **Skill 自治**：skill 自己决定有没有事做（查 watermark、early return），框架只负责按时调度
3. **Scanner 是可选优化**：高频检测场景可配置 scanner 做轻量 gate 预检，默认不需要
4. **熵减优先**：每个 reflection skill 的净效果必须是收敛的（信息量只减不增或持平）
5. **有界执行**：扫描范围、输出数量、LLM 调用次数都有硬上限
6. **故障透明**：任何步骤失败都不推进 watermark，不丢数据，下次自动重试
7. **通知自决**：是否给用户发消息由 skill 自己决定（按需调用 `context.mailbox`），框架不干预
...
可选路径（配置 scanner）：
  Heartbeat 按 schedule 调度
    → skill task 到期 → scanner.check(watermark) 轻量预检
      → 无变化 → 记录 skill_skipped 事件，跳过（不构建 SkillContext）
      → 有变化 → 构建 SkillContext（含 scan_result）→ 调用 skill
        → skill 复用 scan_result，跳过重复查询
...
if task.skill:
    scan_result = None
    # 可选：scanner gate 预检（仅当配置了 scanner 时）
    ...
    if scanner:
        ...
        if not scan_result.has_changes:
            ...
            continue
    # 执行 skill（scan_result 可能为 None，skill 自行处理）
...
| 文件 | 改动 |
|------|------|
| `src/everbot/core/tasks/task_manager.py` | Task dataclass 新增 `skill`、`scanner`（可选）、`min_execution_interval`（可选）字段 |
| `src/everbot/core/runtime/heartbeat.py` | 新增 `_invoke_skill_task()`、`_build_skill_context()`、`_get_scanner()`、可选 scanner gate 逻辑 |
...
| 2 | `ReflectionState` | 无 |
| 3 | `BaseScanner` + `ScanResult` + `SessionScanner` | SessionPersistence |
| 4 | Task 新增 `skill` 字段 + Heartbeat `_invoke_skill_task()` | SkillContext |
...
| 8 | 可选 scanner gate 逻辑 + `_get_scanner()` | BaseScanner |
