...
def build_isolated_task_prompt(task: Any) -> str:
    ...
    return (
        ...
        "FAIL-FAST (#153): If a skill script/tool returns a clear failure "
        "(non-zero exit, empty result, or codes like SELECTOR_OR_STRUCTURE_CHANGED / "
        "LOGIN_REQUIRED), STOP immediately. Do NOT burn the tool budget on shell "
        "archaeology: no rewriting scrapers, no probing DOM selectors, no ad-hoc tsx/curl "
        "debugging loops. Report the failure reason to the user and end the turn.\n"
        "PROVENANCE(溯源, #124 L2): when your report states a specific fact or number that "
